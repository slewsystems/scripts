...
VERSION_API_JSON="https://googlechromelabs.github.io/chrome-for-testing/known-good-versions-with-downloads.json"
...
function main {
  ...
  if [[ -z "$VERSION" || -z "$PLATFORM" ]]; then
    echo_error "Usage: $0 [install|search] --version <version> --platform <platform>"
    exit 1
  fi
